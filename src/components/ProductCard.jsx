...
export function ProductCard({ product, index = 0, forceCol = false }) {
    const router = useRouter();
    const reduce = useReducedMotion();
    const { items, add } = useCart();
    const { has, toggle } = useWishlist();
    const added = items.some((i) => i.slug === product.slug);
    ...
    return (
        ...
        >
            <Link href={`/product/${product.slug}`} className={`flex flex-1 gap-5 md:gap-0 w-full ${forceCol ? 'flex-col' : 'flex-row md:flex-col'}`}>
                {/* Image */}
                <div className={`relative aspect-square overflow-hidden bg-neutral-50/65 shrink-0 ${forceCol ? 'w-full h-auto rounded-none' : 'rounded-xl md:rounded-none w-28 h-28 sm:w-32 sm:h-32 md:w-full md:h-auto'}`}>
                    ...
                    {img ? (
                        ...
                        <img src={img} alt={product.name} loading="lazy" className={`relative h-full w-full object-contain transition-transform duration-700 ease-out group-hover:scale-105 ${forceCol ? 'p-4' : 'p-0 sm:p-0.5 md:p-1.5'}`} />
                    ) : (
                ...
                <div className={forceCol ? 'hidden' : 'flex flex-1 flex-col justify-between md:hidden pt-2.5 pb-2.5 pl-1 pr-1.5 relative'}>
                    <div className="flex justify-between items-start w-full">
                        ...
                        <button onClick={handleWishlist} aria-label={isWished ? 'Remove from wishlist' : 'Add to wishlist'} className="text-neutral-400 hover:text-neutral-800 transition-colors duration-300 shrink-0 -mt-1 -mr-1">
                    ...
                    <div className="flex justify-between items-end w-full mt-2">
                        ...
                        <button onClick={handleAdd} className={`px-3 py-1.5 -mr-3 rounded-lg border text-[10px] font-bold flex items-center gap-1.5 transition-all duration-300 shadow-sm ${added ? 'border-neutral-950 bg-neutral-950 text-white' : 'border-neutral-200 bg-neutral-50 text-neutral-600 hover:bg-neutral-100 hover:text-neutral-900 hover:border-neutral-300'}`}>
